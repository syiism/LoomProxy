...
def build_chapter_item(item: dict[str, Any]) -> ChapterItem:
    return ChapterItem(
        ...
        itemId=str(item.get("cid", "")),
        chapterInfo=item.get("publishTime", ""),
    )


@HandlerRegistry.register
class QQLuomuChapterHandler(ChapterBaseHandler):
    path = "/qq_luomu/chapter"
    name = "qq_luomu_chapter"
    methods = ["GET"]
    query_params = ["base_url", "book_id"]
    description = "qq阅读（落幕）章节"

    async def handle(self, **kwargs: Any) -> ChapterResponse:
        base_url = normalize_api_base(kwargs.get("base_url", ""), "/api")
        book_id = kwargs.get("book_id", "")

        url = f"{base_url}/directory?source=QQ阅读&book_id={book_id}&tab=小说"
