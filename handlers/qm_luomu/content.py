...
class QmLuomuContentHandler(ContentBaseHandler):
    path = "/qm_luomu/content"
    name = "qm_luomu_content"
    methods = ["GET"]
    query_params = ["base_url", "book_id", "item_id"]
    description = "七猫（落幕）正文"
    ...
    async def handle(self, **kwargs: Any) -> ContentResponse:
        ...
        url = f"{base_url}/content?source=七猫&book_id={book_id}&item_id={item_id}&tab=小说"
        try:
